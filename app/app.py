...
# Routes (Blueprints)
from routes.home import home_bp, TeaTimeTips
from routes.about import about_bp
from routes.passwordgen import passwordgen_bp
from routes.whois import whois_bp
from routes.databreach import databreach_bp, init_databreach_routes
from routes.mac_lookup import mac_lookup_bp
from routes.ip_geolocation import ip_geolocation_bp, init_ip_geolocation_routes
...
app.config["SESSION_COOKIE_SAMESITE"] = (
    "Lax"  # Protects against CSRF in cross-origin requests
)
app.config["APP_ENV"] = "production"
app.config["SQLALCHEMY_POOL_SIZE"] = 10
app.config["SQLALCHEMY_POOL_TIMEOUT"] = (
    30  # Adjust pool size and timeout based on your traffic
)
app.config["DEBUG"] = False  # This is already set


# Handle CSRF errors globally for JSON responses
...
# Add this route to serve sitemap.xml
@app.route("/sitemap.xml", methods=["GET"])
def sitemap():
    """Serve the sitemap.xml file."""
    return send_from_directory(
        os.path.join(app.root_path, ""), "sitemap.xml", mimetype="application/xml"
    )
