    ...
    try:
        ...
        if algorithm not in supported_algorithms:
            return (
                jsonify(
                    {
                        "error": "Invalid hashing algorithm. Supported: MD5, SHA-1, SHA-256, SHA-512, SHA3-256"
                    }
                ),
                400,
            )
        ...
        return jsonify(
            {"hash": hash_value, "algorithm": algorithm, "metadata": metadata}
        )

    except Exception as e:
        # Log the exact error causing the 500 Internal Server Error
        print(f"Error: {e}")
