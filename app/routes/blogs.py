...
def add_blog():
    ...
    response = make_response(
        render_template("/blogs/add_blog.html", form=form, api_key=api_key)
    )
...
def blog_detail(blog_id):
    ...
    return render_template(
        "/blogs/blog_detail.html", title=blog_post["title"], blog_post=blog_post
    )
...
def edit_blog(blog_id):
    api_key = "7dowqsizhjpg0668xmzqpw2d4c8by4dam96yft0stapvilcc"  # for tinymce blyat. if you are hacker, then fuck me i let this go in the production lol .-.
    ...
    cursor.execute(
        "SELECT id, title, summary, content, image, category_id FROM posts WHERE id = ?",
        (blog_id,),
    )
    ...
    response = make_response(
        render_template(
            "/blogs/edit_blog.html", form=form, blog_id=blog_id, api_key=api_key
        )
    )
    response.headers["Content-Security-Policy"] = (
        "default-src 'none'; "
        "script-src 'self' *.tinymce.com *.tiny.cloud https://cdn.jsdelivr.net; "
        "connect-src 'self' *.tinymce.com *.tiny.cloud blob: https://cdn.jsdelivr.net; "
        "img-src 'self' *.tinymce.com *.tiny.cloud data: blob: https://cdn.jsdelivr.net; "
        "style-src 'self' 'unsafe-inline' *.tinymce.com *.tiny.cloud https://cdn.jsdelivr.net; "
        "font-src 'self' *.tinymce.com *.tiny.cloud;"
    )
